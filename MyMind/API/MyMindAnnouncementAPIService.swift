...
import RxSwift
import PromiseKit
class MyMindAnnouncementAPIService: PromiseKitAPIService, AnnouncementListLoader {
    ...
    func announcements(with info: AnnouncementListQueryInfo? = nil) -> Promise<AnnouncementList> {
        guard let userSession = userSessionDataStore.readUserSession() else {
            return .init(error: APIError.noAccessTokenError)
        }
